...
import { Plus, Filter, Search, CheckSquare, Sword, Clock, Star, TrendingUp, Play, Square, Timer, CalendarPlus } from 'lucide-react'
          ...
          <div className="flex gap-4 mb-6">
            <div className="flex-1 relative">
              ...
              <input
                ...
                className="w-full pl-10 pr-4 py-2 bg-theme-primary border border-gray-800 rounded-lg focus:outline-none focus:border-theme-accent"
              />
            </div>
            <button className="flex items-center gap-2 px-4 py-2 bg-theme-primary border border-gray-800 rounded-lg hover:bg-theme-bg transition-colors">
      ...
      {activeTab === 'chains' && (
        <QuestChains
          ...
          onStartChainQuest={(questData) => {
            // Create quest from chain template
            createTask(questData).then(() => {
    ...
    <div className="bg-theme-primary rounded-lg border border-gray-800 p-4 hover:border-gray-700 hover:shadow-lg hover:shadow-solo-accent/10 transition-all duration-300 hover:-translate-y-1">
      <div className="flex items-start justify-between">
        <div className="flex-1">
          ...
          {/* Recurring quest info */}
          {task.task_type === 'recurring' && task.recurrence_pattern && (
            <div className="mb-3 p-2 bg-theme-bg rounded-lg border border-gray-700">
              <div className="flex items-center gap-2 text-xs">
                <span className="text-lg">{getRecurringQuestIcon(task.recurrence_pattern)}</span>
                <div className="flex-1">
                  <div className="text-theme-accent font-medium">
                    {formatRecurrencePattern(task.recurrence_pattern)}
                  </div>
                  <div className="text-gray-400">
                    {task.current_streak ? `${task.current_streak} day streak` : 'No streak yet'}
                    {task.total_completions ? ` • ${task.total_completions} completions` : ''}
                  </div>
                </div>
                {isQuestDueToday(task) && (
